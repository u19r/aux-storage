...
//! # Guide
//!
//! This library provides [`Connection`] struct. [`Connection`] struct is a
//! handle to call functions in background thread and can be cloned cheaply.
...
#[non_exhaustive]
pub enum Error {
    /// The connection to the SQLite has been closed and cannot be queried any
    /// more.
    ConnectionClosed,

    /// An error occured while closing the SQLite connection.
    /// This `Error` variant contains the [`Connection`], which can be used to
    /// retry the close operation and the underlying [`rusqlite::Error`]
    /// that made it impossile to close the database.
...
impl Connection {
    ...
    /// asynchronously.
    ///
    /// This method can cause a `panic` if the underlying database connection is
    /// closed. it is a more user-friendly alternative to the
    /// [`Connection::call`] method. It should be safe if the connection is
    /// never explicitly closed (using the [`Connection::close`] call).
    ///
    ...
    /// [`Error::ConnectionClosed`], and any calls to
    /// [`Connection::call_unwrap`] will cause a `panic`.
    ///
    /// # Failure
    ///
    ...
    pub async fn close(self) -> Result<()> {
        ...
        if let Err(crossbeam_channel::SendError(_)) = self.sender.send(Message::Close(sender)) {
            // If the channel is closed on the other side, it means the connection closed
            // successfully This is a safeguard against calling close on a
        ...
        if result.is_err() {
            // If we get a RecvError at this point, it also means the channel closed in the
            // meantime we can assume the connection is closed
            return Ok(());
        }
...
where F: FnOnce() -> rusqlite::Result<rusqlite::Connection> + Send + 'static {
    let (sender, receiver) = crossbeam_channel::unbounded::<Message>();
