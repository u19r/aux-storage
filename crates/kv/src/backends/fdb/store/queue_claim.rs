...
use crate::{
    backends::fdb::{
        ...
        store::{FoundationDbKvStore, read_fdb_keys_concurrently, rotate_fdb_claim_candidates},
    },
    queue::{
...
const QUEUE_READY_RANGE_CONCURRENCY: usize = 8;

async fn read_queue_ready_range(
    trx: &foundationdb::Transaction,
    prefix: Vec<u8>,
    range: QueueClaimRange,
) -> Result<(QueueClaimRange, Vec<(Vec<u8>, Vec<u8>)>), foundationdb::FdbError> {
    let start = FoundationDbKvStore::prefix_bytes(&prefix, &range.ready_start);
    let end = FoundationDbKvStore::prefix_bytes(&prefix, &range.ready_end);
    ...
    let entries = trx.get_range(&option, 1, true).await?;
    let entries = entries
        .into_iter()
...
async fn read_queue_ready_range_window(
    trx: &foundationdb::Transaction,
    prefix: &[u8],
    ranges: &[QueueClaimRange],
) -> Result<Vec<(QueueClaimRange, Vec<(Vec<u8>, Vec<u8>)>)>, foundationdb::FdbError> {
    let prefix = prefix.to_vec();
    try_join_all(
        ranges
            .iter()
            .cloned()
            .map(|range| read_queue_ready_range(trx, prefix.clone(), range)),
    )
    .await
}
...
impl FoundationDbKvStore {
    pub(crate) async fn claim_queue_messages_from_ranges_operation(
        ...
        'retry: loop {
            ...
            for range_window in ranges.chunks(QUEUE_READY_RANGE_CONCURRENCY) {
                if batch.messages.len() >= max_claims {
                    break;
                }
                let ready_range_entries =
                    match read_queue_ready_range_window(&trx, prefix, range_window).await {
                        Ok(entries) => entries,
                        Err(err) => {
                            trx = self
                ...
                for (range, mut ready_entries) in ready_range_entries {
                    let start = Self::prefix_bytes(prefix, &range.ready_start);
                    let end = Self::prefix_bytes(prefix, &range.ready_end);
                    read_key_bytes =
                    ...
                    read_bytes = read_bytes.saturating_add(
                        ready_entries
                            .iter()
                            .map(|entry| entry.0.len().saturating_add(entry.1.len()) as u64)
                            .sum::<u64>(),
                    );
                    batch.ready_entries_seen =
                        batch.ready_entries_seen.saturating_add(ready_entries.len());
                    if batch.messages.len() >= max_claims || ready_entries.is_empty() {
                        continue;
                    }
                    ...
                    let candidate_ready_keys = candidates
                        ...
                        .collect::<Vec<_>>();
                    let ready_reads =
                        match read_fdb_keys_concurrently(&trx, candidate_ready_keys, false).await {
                            Ok(ready_reads) => ready_reads,
                            Err(err) => {
                                // The owned helper consumes key buffers; these keys are only
                                // diagnostic context, so avoid rebuilding them on a failed read.
                                trx = self
                                    .retry_transaction_after_fdb_error(
                                        trx,
                                        "queue_claim",
                    ...
                    for (ready_entry, ready_value) in candidates.into_iter().zip(ready_reads) {
                        ...
                        read_bytes =
                            read_bytes.saturating_add(
                                ready_entry.0.len().saturating_add(
                                    ready_value.as_ref().map_or(0, |value| value.len()),
                                ) as u64,
                            );
                        read_key_bytes = read_key_bytes.saturating_add(ready_entry.0.len() as u64);
                        let Some(ready_value) = ready_value else {
                            continue;
                        };

                        let ready_key = self.strip_prefix(&ready_entry.0).to_vec();
                        ...
                        let prefixed_state_key = Self::prefix_bytes(prefix, &state_key);
                    ...
                    let prefixed_state_keys = claim_candidates
                        ...
                        .collect::<Vec<_>>();
                    let state_reads =
                        match read_fdb_keys_concurrently(&trx, prefixed_state_keys, true).await {
                    ...
                    for (ready_candidate, state_bytes) in
                        claim_candidates.into_iter().zip(state_reads)
                    {
                        let (
                            ready_key,
                            ready_value,
                        ...
                        read_key_bytes =
                            read_key_bytes.saturating_add(prefixed_state_key.len() as u64);
                        ...
                        let prefixed_new_ready_key = Self::prefix_bytes(prefix, &new_ready_key);
                    ...
                    {
                        let prefixed_hint_key = Self::prefix_bytes(prefix, &range.ready_hint_key);
                        let hint_value = crate::partition_family::queue_ready_hint_bytes(
                            range.partition_id,
                            now,
                        );
            ...
            match trx.commit().await {
                Ok(_) => {
                    ...
                    let prefixed_payload_keys = pending_claims
                        .iter()
                        .map(|(payload_key, _)| Self::prefix_bytes(prefix, payload_key))
                        .collect::<Vec<_>>();
                    let payload_reads =
                        read_fdb_keys_concurrently(&payload_trx, prefixed_payload_keys, true)
                            .await
