...
pub fn create_item_update_stream_entries_wire_encoded(
    ...
    let stored_pointer = if should_embed_stream_items(item_bytes, old_item_bytes) {
        ...
        CompactStoredStreamPointer::embedded(
            ...
            items,
            replication.cloned(),
            context.indexers.to_vec(),
            context.old_indexers.map(<[_]>::to_vec),
        )
    } else {
    ...
    let pointer_binding = UniquePlaceholderBinding::new(fallback);
    let compact_keys =
    ...
    Ok(vec![
        (
            KeyTemplate::unique_placeholder(
                compact_keys.system_row,
                Vec::new(),
                pointer_binding.clone(),
            ),
            pointer_bytes.clone(),
        ),
        (
            KeyTemplate::unique_placeholder(
                compact_keys.table_row,
                Vec::new(),
                pointer_binding.clone(),
            ),
            pointer_bytes,
