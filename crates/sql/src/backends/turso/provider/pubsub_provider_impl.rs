...
use crate::backends::turso::provider::{TursoStorageProvider, row_required_text};
...
#[async_trait]
impl PubsubProvider for TursoStorageProvider {
    async fn initialize(&self) -> PubsubResult<()> {
        let _ddl_guard = self.ddl_lock.lock().await;
        let conn = self.connect().await.map_err(PubsubError::storage)?;
        self.execute(
            &conn,
...
impl TursoStorageProvider {
    ...
    async fn put_text(&self, key: &str, value: &str) -> PubsubResult<()> {
        ...
        self.execute(
            &conn,
            "INSERT INTO sys_pubsub_kv (key, value) VALUES (?1, ?2)
             ON CONFLICT(key) DO UPDATE SET value = excluded.value",
            vec![
                TursoValue::Text(key.to_string()),
    ...
    async fn get_text(&self, key: &str) -> PubsubResult<Option<String>> {
        ...
        let rows = self
            .query_rows(
                &conn,
                "SELECT value FROM sys_pubsub_kv WHERE key = ?1",
                vec![TursoValue::Text(key.to_string())],
            )
            .await
            .map_err(PubsubError::storage)?;
        rows.into_iter()
            .next()
            .map(|row| row_required_text(&row, "value").map_err(PubsubError::storage))
            .transpose()
    }
    ...
    async fn delete_key(&self, key: &str) -> PubsubResult<()> {
        ...
        self.execute(
            &conn,
            "DELETE FROM sys_pubsub_kv WHERE key = ?1",
            vec![TursoValue::Text(key.to_string())],
        )
        .await
        .map_err(PubsubError::storage)?;
    ...
    async fn scan_text(&self, prefix: &str) -> PubsubResult<Vec<String>> {
        ...
        let rows = self
            .query_rows(
                &conn,
                "SELECT value FROM sys_pubsub_kv WHERE key LIKE ?1 ORDER BY key",
