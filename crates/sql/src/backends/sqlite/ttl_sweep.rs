...
use storage_common::{
    TTL_SWEEP_JOB, TtlSweepLock,
    retry::{RetryPolicy, execute_with_retry},
    ttl::{TtlConfigRecord, normalize_ttl_seconds, ttl_index_key_map_from_token},
};
use storage_provider::StorageProvider;
use storage_types::{
    BatchWriteItemRequest, DeleteRequest, StorageError, StorageResult, StoredTableInfo, TableName,
    TimeToLiveStatus, TimestampMillis, WriteRequest,
};
use tracing::{Instrument, debug, info, warn};
use uuid::Uuid;

use crate::{
    SQLiteStorageProvider, constants, naming, stream_writer::should_write_stream_entries,
    utils::call_sqlite,
};

pub struct TtlSweepJob {
...
#[async_trait]
impl BackgroundJob for TtlSweepJob {
...
impl SQLiteStorageProvider {
    #[tracing::instrument(level = "debug", skip_all, fields(job = %TTL_SWEEP_JOB))]
    pub(crate) async fn run_ttl_sweep(&self) -> StorageResult<bool> {
        ...
        let mut throttled_tables = 0usize;
        let mut total_retry_batches = 0u64;
        let mut total_retry_attempts = 0u64;
        let mut total_retry_failures = 0u64;
        ...
        for (table_name, mut config) in configs {
            ...
            adjust_ttl_shard_batch(&table_name, &mut config, runtime_ms);
        ...
        .increment(processed_tables as u64);
    ...
    async fn try_acquire_ttl_lock(
        ...
        let Some(mut refreshed_config) = refreshed else {
            warn!(table = %table_name, "ttl.sweep.lock_state_missing_after_save");
            return Ok(None);
        };
    ...
    async fn process_ttl_for_table(
        ...
        let now_normalized = i64::try_from(normalize_ttl_seconds(now_seconds))
            .map_err(|_| StorageError::internal("sqlite ttl sweep normalize overflow"))?;
        let mut deleted_items = 0usize;
        let mut shards_checked = 0usize;
        let mut throttled = false;
        let mut last_processed_watermark = config.last_processed_watermark;
        let mut retry_batches = 0u32;
        let mut retry_attempts = 0u32;
        let mut retry_failures = 0u32;
        let should_write_to_stream = should_write_stream_entries(table_info);
        ...
        for batch_index in 0..shard_batch {
            let rows = self
                .fetch_ttl_index_rows(
                    table_name,
                    now_normalized,
                    constants::TTL_SWEEP_ITEMS_PER_SHARD,
                )
                .await?;
            ...
            if !rows.is_empty() {
                deleted_items = deleted_items.saturating_add(rows.len());
                let batch_capacity = rows
                    .len()
                    .saturating_add(constants::TTL_SWEEP_DELETE_BATCH_SIZE - 1)
                    / constants::TTL_SWEEP_DELETE_BATCH_SIZE;
                let mut delete_batches = Vec::with_capacity(batch_capacity);
                let mut batch_writes = Vec::with_capacity(constants::TTL_SWEEP_DELETE_BATCH_SIZE);

                for row in &rows {
                    let key_map = ttl_index_key_map_from_token(&row.key_token, table_info)?;
                    batch_writes.push(WriteRequest {
                        put_request: None,
                    ...
                    if batch_writes.len() == constants::TTL_SWEEP_DELETE_BATCH_SIZE {
                        let mut request_items = HashMap::with_capacity(1);
                        request_items.insert(table_name.clone(), std::mem::take(&mut batch_writes));
                ...
                if !batch_writes.is_empty() {
                    let mut request_items = HashMap::with_capacity(1);
                    request_items.insert(table_name.clone(), batch_writes);
                    delete_batches.push(BatchWriteItemRequest {
                        request_items,
                        return_consumed_capacity: None,
                        return_item_collection_metrics: None,
                ...
                for (result, attempts, failures) in results {
            ...
            if rows.len() == constants::TTL_SWEEP_ITEMS_PER_SHARD && batch_index + 1 >= shard_batch
            {
                throttled = true;
                break;
            }
    ...
    async fn fetch_ttl_index_rows(
        &self,
        table_name: &TableName,
        now_seconds: i64,
        limit: usize,
    ) -> StorageResult<Vec<TtlIndexRow>> {
        let ttl_table = naming::physical_ttl_index_table_name(table_name);
        let limit = i64::try_from(limit).unwrap_or(i64::MAX);
        let sql = format!(
            "SELECT ttl_value, key_token FROM \"{ttl_table}\" WHERE ttl_value <= ?1 ORDER BY \
             ttl_value ASC LIMIT ?2"
        );

        call_sqlite(&self.connection, move |conn| {
            let mut stmt = conn.prepare(&sql).map_err(map_sqlite_error)?;
            let mut rows = stmt
                .query(rusqlite::params![now_seconds, limit])
                .map_err(map_sqlite_error)?;
            let mut result = Vec::new();
            while let Some(row) = rows.next().map_err(map_sqlite_error)? {
                let ttl_value: i64 = row.get(0).map_err(map_sqlite_error)?;
                let key_token: String = row.get(1).map_err(map_sqlite_error)?;
                result.push(TtlIndexRow {
                    ttl_value,
                    key_token,
                });
            }
